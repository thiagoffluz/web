...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Solicita ao servidor a lista de todos os produtos, porém de forma assíncrona
    ...
    //pega a lista recebida (array javascipt) recebida e processa
    .then(lista => {
        if (lista.length == 0)
    ...
    const adicionarNaTabela = (lista) => {
        ...
        lista.forEach(produto => {
            // calcula o numero da linha atual da tabela
            const tamanhoTabela = tabela.rows.length

            // insere uma linha embaixo da outra
            ...
            if (url.substring(0, 4) === 'http') 
                imagem.innerHTML = `<img src='${url}' width = '50px' class='img-thumbnail' alt='Foto do Produto'>`
            // imagem da pasta galeria
            else
                imagem.innerHTML = `<img src='galeria/${url}' width = '50px' class='img-thumbnail' alt='Foto do Produto'>`
    ...
    const excluirDaTabela = (id) => {
        ...
        // envia solicitação para o servidor deletar o produto id = ?
