document.addEventListener('DOMContentLoaded', () => {
    ...
    // Exemplo: http://localhost:5500/alterar.html?id=10
    const urlParams = new URLSearchParams(window.location.search) // id = 10
    const id = urlParams.get('id')

    // solicita ao servidor o produto com id = 10
    ...
    .then(data => {
        if (data.length > 0){
            ...
            alerta_erro(`Errp: nenhum produto encontrado com id=${id}`)
...
const atualizar = () => {
    ...
    // pega o objeto json retornado pelo banco
    ) .then(data => {
